...
# The pipeline feeds a binary `relapse` column into mlxtend's apriori, which
...
# conventions are handled below (see run_apriori.py one-hot re-encoding).
...
def process_cf_positive_filter(method, rules_path, out_dir):
    ...
    re-run Apriori. CF <= 0 means confidence does not beat the consequent's
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter Apriori rules to class-association rules and "
    ...
    parser.add_argument(
        ...
        "as --base-dir). Use e.g. data/final/ARM/apriori_v3 to keep the "
